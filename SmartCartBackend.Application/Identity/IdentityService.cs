    ...
    IHttpContextAccessor contextAccessor) : IIdentityService
{
        ...
        CancellationToken ct = default)
    {
        ...
        userContext.ActivityLevel = "Умеренная активность";
        userContext.Allergies = user.Allergies.Select(x => x.Allergy.Title).ToList();
        userContext.Intolerances = user.Intolerances.Select(x => x.Intolerance.Title).ToList();
        userContext.Preferences = user.Preferences.Select(x => x.Preference.Title).ToList();
